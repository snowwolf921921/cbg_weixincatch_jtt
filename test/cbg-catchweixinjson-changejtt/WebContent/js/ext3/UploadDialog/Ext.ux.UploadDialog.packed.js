...
Ext.ux.Utils.EventQueue=function(handler,scope){
	...
	this.postEvent=function(event,data){
		...
		if(!this.is_processing){this.process()}
	};
...
Ext.extend(Ext.ux.Utils.FSA,Ext.ux.Utils.EventQueue,{
	...
	processEvent:function(event,data){
		...
		for(var i=0,len=transitions.length;i<len;i++){
			...
			if(this.computePredicate(predicate,scope,data,event)){
				...
				this.current_state=new_state;return 
			}
		}
	...
	computePredicate:function(predicate,scope,data,event){
		...
		switch(Ext.type(predicate)){
			case"function":result=predicate.call(scope,data,event,this);
			break;
			...
			break;
			case"boolean":result=predicate;
			break;
			default:throw ["Predicate: ",predicate,' is not callable in "',this.current_state,'" state for event "',event].join("")
		}
		return result
	},
	callAction:function(action,scope,data,event){
		switch(Ext.type(action)){
			case"array":for(var i=0,len=action.length;i<len;i++){
			...
			break;
			case"function":action.call(scope,data,event,this);
			break;
			default:throw ["Action: ",action,' is not callable in "',this.current_state,'" state for event "',event].join("")
		}
	}
});
...
Ext.extend(
	...
	Ext.Window,
	{
		...
		initComponent:function(){
			...
			var tt={
				...
				"ready":{
					...
					"destroy":[{
						action:this.flushEventQueue,state:"destroyed"
					}]
				},
				"adding-file":{
					"file-added":[{
				...
				"uploading":{
					"file-selected":[{
						...
						action:this.addFileToUploadQueue,state:"adding-file"
					},{}],
					"grid-selection-change":[{}],
					"start-upload":[{}],
					"stop-upload":[{
		...
		createForm:function(){
			this.form=Ext.DomHelper.append(
				this.body,
				{
					tag:"form",
					method:"post",
		...
		fillToolbar:function(){
			...
			tb.x_buttons.reset=tb.addButton({
				text:this.i18n.reset_btn_text,tooltip:
				this.i18n.reset_btn_tip,
			...
			tb.x_buttons.upload=tb.addButton({
				...
				handler:this.onUploadButtonClick,scope:this
			});
			tb.add("-");
			tb.x_buttons.indicator=tb.addItem(
				new Ext.Toolbar.Item(Ext.DomHelper.append(
					tb.getEl(),
					{tag:"div",cls:"ext-ux-uploaddialog-indicator-stoped",html:"&#160"}
				))
			);
			tb.add("->");
		...
		getFileExtension:function(filename){
			...
			if(parts.length>1){result=parts.pop()}
			return result
		},
		...
		addFileToUploadQueue:function(browse_btn){
			...
			//同一次上传中，不允许相同文件名的文件上传
			var uploadedFileName,uploadingFileName;
			for(var i=0;i<store.getCount();i++){
				uploadedFileName=store.getAt(i).get('filename').substring(store.getAt(i).get('filename').lastIndexOf("\\")+1,store.getAt(i).get('filename').length);
				uploadingFileName=input_file.dom.value.substring(input_file.dom.value.lastIndexOf("\\")+1,input_file.dom.value.length);
				if(uploadedFileName==uploadingFileName){
					Ext.MessageBox.show({
						title:'失败',
						msg: '上传文件名重复，请重新选择！',
		...
		updateToolbar:function(){
			...
			if(this.is_uploading){
				...
				tb.x_buttons.upload.setText(
					this.i18n.upload_btn_start_text
				);
				tb.x_buttons.upload.getEl().child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType]
		...
		resetQueue:function(){
			var store=this.grid_panel.getStore();
			store.each(function(r){r.get("input_element").remove()});
			store.removeAll()
		},
		...
		//修改了一下，上传之后更新数据；
		//20090206 fyh
		//调用上传文件的控件时，通过base_params传两个参数，一个是附件ID，一个是显示附件信息的form
		onCloseButtonClick:function(){
			var params = this.getBaseParams().params;
			var UploadForm = params.UploadForm;
			var ID = params.UploadId;
			if(ID){
				if(UploadForm&&UploadForm.findById('files')){
					UploadForm.remove(UploadForm.findById('files'));
				}
				Ext.Ajax.request({
					url: 'execQuery.do?funcId=9072&resultType=0&30240='+ID,
					success: function(response){
						...
						if(UploadForm){
							...
							UploadForm.add(fileS);
							UploadForm.doLayout();
							UploadForm.findById('files').getEl().dom.appendChild(document.createTextNode('附件：'));
						}else{
							var fileArr = [];
						}
			    		for( i = 0;i < selectArray.length;i ++ ){
			    			var attach_up_id= selectArray[i][0];
			    			var attach_name = selectArray[i][1];
			    			var attach_type = selectArray[i][2];
			    			var attach_id= selectArray[i][3];
			    			var size=0.00;
	    					size = Math.round((Math.floor(selectArray[i][2]/1024*1000)/10))/100;//大小
							if(UploadForm){
								var a = document.createElement('a');
								a.setAttribute("href",'javascript:openfile("'+attach_up_id+'","'+attach_name+'")');
								a.setAttribute("title",'附件_'+(i+1));
								a.appendChild(document.createTextNode(attach_name ));
								var img = document.createElement('img');
								img.setAttribute('src','images/delete.gif');
								//javascript中闭包的应用
								var delfilefn = new delfile(attach_id,UploadForm);
								img.onclick = delfilefn.clickfunc;
								UploadForm.findById('files').getEl().dom.appendChild(a);
								UploadForm.findById('files').getEl().dom.appendChild(img);
								if(i!=selectArray.length-1){
				    				UploadForm.findById('files').getEl().dom.appendChild(document.createTextNode(','));
				    			}
							}else{
								/*var data={}
								data.fid=fid;
			    		...
			    		if(UploadForm){
			    			...
			    			//如果调用附件时，没有传form的参数，则在关闭窗口时，调用该函数，该函数调用者自己定义。
							if(typeof(showUploadFileWithoutForm)=='function'){
								//showUploadFileWithoutForm(fileArr);
							}
			    		}
		...
		onAjaxSuccess:function(response,options){
			var json_response={"success":false,"error":this.i18n.note_upload_error};
			try{
				var result = response.responseText;
				/*if(result.indexOf('<pre>')>0);{
					result = result.substring(0,result.indexOf('<pre>'))+result.substring(result.indexOf('<pre>')+5,result.length);
				}
				if(result.indexOf('</pre>')>0);{
					result = result.substring(0,result.indexOf('</pre>'))+result.substring(result.indexOf('</pre>')+6,result.length);
				}*/
				var json_response=Ext.util.JSON.decode(result);
			}catch(e){
				
			}
			var data={record:options.record,response:json_response};
		...
		onAjaxFailure:function(response,options){
			var data={record:options.record,response:{"success":false,"error":this.i18n.note_upload_failed}};
