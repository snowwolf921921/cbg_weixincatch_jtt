...
//		sortDir : null,
    ...
    doRequest : function(action, rs, params, reader, callback, scope, options){
        params = params ||
        {};
//        if (!this.data||this.data.length==0){this.data=params.data}
//        this.data=params.data;
        var result;
        try {
        	
        	if(params.refresh !== undefined){
				this.data = params.data.slice(0);
				this.sortField = null;
				this.sortDir = null;
			}
			if((this.buffer.length == 0)||(params.refresh !== undefined)){
				result = reader.readRecords(this.data);
			}
			else
				result = {records : this.buffer,totalRecords : this.buffer.length};
